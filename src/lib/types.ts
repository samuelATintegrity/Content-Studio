...
// ── Funny Commercial workflow ────────────────────────────────────────
//
// A 4-scene comedic ad template. Only Scene 1 is fresh per ad — the
// remaining scenes are picked from per-scene libraries. See
// src/lib/prompts/funny-commercial-themes.ts for the Scene 1 theme
// library and src/lib/funnyCommercialLibrary.ts for the storage
// helpers.
...
export type FcTheme = {
  key: string;
  premise: string;        // human-readable comedic premise this theme conveys
  tone: string;           // voice nudge for any companion copy
  visuals: FcVisual[];
};

// One image generated by Nano Banana for a Scene 1 idea.
export interface FcSceneImage {
  id: string;
...
// One animated clip — either Scene 1 (animation of an FcSceneImage) or
// a Scene 2 actor clip (independent of Scene 1).
export interface FcSceneVideo {
  id: string;
  url: string;
  sourceImageId?: string; // backref to the FcSceneImage, when applicable
  imagePrompt?: string;   // recorded for display + cooldown bookkeeping
  themeKey?: string;
  conceptKey?: string;
  savedAt: number;
  // "ai" = produced via Nano + Seedance; "upload" = user-uploaded mp4.
  source?: "ai" | "upload";
  filename?: string;      // for uploads, original filename for display
}

// Saved Scene 2 text overlay phrase. Phrases are language-tagged so the
// modal can filter by current sidebar language.
export interface FcPhrase {
  id: string;
  text: string;
  language: Language;
  savedAt: number;
...
// Saved Scene 3 CTA. Authored in English, translations cached on first
// use so subsequent renders are free + reviewable.
export interface FcTranslatedPhrase {
  id: string;
  en: string;
  tl?: string;
  es?: string;
...
// Render payload for the worker — passed through /api/video/render.
export interface FcRenderRequest {
  scene1VideoUrl: string;
  scene2VideoUrl: string;
  scene2Text: string;
  scene3Text: string;       // already translated to the active language
  language: Language;
  // Optional overrides — defaults documented in the worker's
  // composeFunnyCommercial implementation.
